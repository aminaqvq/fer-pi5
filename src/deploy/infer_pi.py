...
def tflite_infer(interpreter, in_det, out_det, x_float):
    ...
    if probs.size != len(LABELS):
        print(f"[WARN] Model output size {probs.size} != expected {len(LABELS)}")
...
def main():
    ...
    yunet = create_yunet(YUNET_PATH, (CAP_W, CAP_H))
    ...
    while True:
        ...
        det_boxes, det_confs = yunet_detect(yunet, frame)
        if len(det_boxes) != 1:
            smooth_queue.clear()
        ...
        cv2.imshow("FER-Pi5 (YuNet + TFLite)", frame)
